...
def test_editor_window_geometry_roundtrip(tmp_path):
    """set/get editor window geometry persists x/y/w/h to the state file."""
    from hushsnap.config import (
        get_editor_window_geometry,
        set_editor_window_geometry,
    )
    state_path = tmp_path / "hushsnap_state.toml"
    assert get_editor_window_geometry(state_path) is None  # nothing yet

    set_editor_window_geometry(120, 80, 1100, 720, state_path)
    assert get_editor_window_geometry(state_path) == {
        "x": 120, "y": 80, "w": 1100, "h": 720,
    }


def test_editor_window_geometry_preserves_other_state(tmp_path):
    """Writing editor geometry must not drop OCR/onboarding state fields."""
    from hushsnap.config import (
        set_editor_window_geometry,
        get_ocr_font_size,
    ...
    set_editor_window_geometry(10, 20, 800, 600, state_path)

    # Both must survive the write.
...
def test_editor_window_geometry_rejects_too_small(tmp_path):
    """A geometry smaller than the minimum is treated as not stored."""
    from hushsnap.config import (
        set_editor_window_geometry,
        get_editor_window_geometry,
    )
    state_path = tmp_path / "hushsnap_state.toml"
    set_editor_window_geometry(0, 0, 100, 100, state_path)
    # Stored on disk, but the getter rejects sub-minimum sizes.
    assert get_editor_window_geometry(state_path) is None
