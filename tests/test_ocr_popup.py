...
# ── click-outside-to-close tests (ActivationChange) ──────────────────────────


def _send_deactivate(popup, monkeypatch):
    """Simulate the popup losing activation (e.g. user clicked outside).

    In headless tests there is no window manager to toggle activation,
    so we monkey-patch ``isActiveWindow`` to return False.
    """
    monkeypatch.setattr(popup, "isActiveWindow", lambda: False)
    QtWidgets.QApplication.sendEvent(
        popup,
        QtCore.QEvent(QtCore.QEvent.Type.ActivationChange),
    )
    QtWidgets.QApplication.processEvents()


def test_deactivate_closes_popup_by_default(qapp, monkeypatch):
    """With the default config (True), losing activation hides the popup."""
    popup = OcrPopup(_translate)
    popup.show_text("hello")
    popup.resize(420, 200)
    popup.show()
    ...
    _send_deactivate(popup, monkeypatch)
    assert not popup.isVisible()


def test_deactivate_does_not_close_when_disabled(qapp, monkeypatch):
    """When the toggle is off, losing activation leaves the popup visible."""
    monkeypatch.setattr(
        "hushsnap.ui.ocr_popup.get_close_ocr_popup_on_focus_loss",
        lambda: False,
    )
    popup = OcrPopup(_translate)
    popup.show_text("hello")
...
def test_deactivate_does_not_close_pinned_popup(qapp, monkeypatch):
    """A pinned popup ignores deactivation regardless of the config toggle."""
    popup = OcrPopup(_translate)
    popup.show_text("hello")
    popup.resize(420, 200)
    popup.show()
