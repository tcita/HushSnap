...
@patch("ctypes.windll.user32.SetActiveWindow")
@patch("ctypes.windll.user32.SetFocus")
...
def test_capture_window_force_topmost_logic(
    ...
    """The hard-fallback escalation attaches threads when the light path fails.

    Covers _force_topmost_hard (the fallback invoked by claim_foreground when
    the ShareX-style light path does not win the foreground). The light path
    itself is Qt raise_/activateWindow and is exercised end-to-end manually.
    """
    from hushsnap.capture_window import _force_topmost_hard

    # Foreground is some OTHER window (0x999); our hwnd is 0x123. The light
    # SetForegroundWindow/SetWindowPos/SetActiveWindow/SetFocus calls inside
    # the hard path run but never win (GetForegroundWindow stays 0x999), so
    # the routine falls through to AttachThreadInput.
    ...
    ok = _force_topmost_hard(0x123, 0x999)

    # AttachThreadInput attached (True) and detached (False) — both happen.
    mock_attach.assert_any_call(100, 200, True)
    mock_attach.assert_any_call(100, 200, False)
    # SetForegroundWindow was attempted for our window.
    mock_set_fg.assert_called()
    # GetForegroundWindow never returned our hwnd, so the routine reports failure.
    assert ok is False
...
def test_capture_window_force_topmost_hung(
    ...
    """Thread attachment is skipped when the old foreground window is hung."""
    from hushsnap.capture_window import _force_topmost_hard

    # Old foreground (0x999) is HUNG; our hwnd (0x123) never wins foreground.
    mock_get_fg.return_value = 0x999
    mock_is_hung.return_value = True

    _force_topmost_hard(0x123, 0x999)

    # AttachThreadInput must NOT be called when the old foreground is hung.
...
def test_capture_window_force_topmost_attach_fail(
    ...
    """When AttachThreadInput fails, SetForegroundWindow is not retried."""
    from hushsnap.capture_window import _force_topmost_hard

    mock_get_fg.return_value = 0x999  # never our hwnd
    mock_is_hung.return_value = False
    mock_get_curr_id.return_value = 100
    mock_get_thread_id.return_value = 200
    mock_attach.return_value = False  # attach fails

    _force_topmost_hard(0x123, 0x999)

    # Attach was attempted (True) but, since it failed, not detached.
    mock_attach.assert_called_with(100, 200, True)
    assert mock_attach.call_count == 1
    # SetForegroundWindow is called once during the soft stage of the hard
    # path; it must NOT be retried in the attach block (attached is False).
