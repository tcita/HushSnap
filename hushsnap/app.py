...
class Application(QtCore.QObject):
    ...
    def _trigger_wer_test_crash(self):
        ...
        # crashlib.dll is bundled by PyInstaller. In --onedir mode it lands in
        # _internal/ (sys._MEIPASS), not next to the exe. Check both.
        import sys as _sys
        candidates = []
        if hasattr(_sys, "_MEIPASS"):
            candidates.append(Path(_sys._MEIPASS) / "crashlib.dll")
        candidates.append(APP_DIR / "crashlib.dll")
